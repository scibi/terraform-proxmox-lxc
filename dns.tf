locals {
  ...
  dns_hostname = split(".", var.ct_name)[0]
  dns_domain   = join(".", slice(split(".", var.ct_name), 1, length(split(".", var.ct_name))))
...
resource "opnsense_unbound_host_override" "forward" {
  ...
  enabled     = true
  description = "Managed by OpenTofu - ${var.ct_name}"
...
resource "opnsense_unbound_host_alias" "cname" {
  ...
  description = "Managed by OpenTofu - CNAME for ${var.ct_name}"
}
